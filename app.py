...
top_quotes = (
    filtered_df.groupby("Marketing Channel")["TotalNumberOfInsuranceQuotes"]
    .sum()
    .idxmax()
)

top_policies = (
    filtered_df.groupby("Marketing Channel")["TotalNumberOfInsurancePoliciesPurchaed"]
    .sum()
    .idxmax()
)
...
with col1:
    st.success(f"👥 Highest number of users came from **{top_users}**.")

    st.success(f"💰 Highest revenue was generated by **{top_revenue}**.")

with col2:
    st.success(f"📝 Most insurance quotes came from **{top_quotes}**.")

    st.success(f"📄 Most insurance policies were purchased through **{top_policies}**.")
